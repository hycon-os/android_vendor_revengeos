...
func init() {
	android.RegisterModuleType("hycon_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hyconExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hyconExpandVariables(ctx, dep_file)
	...
	cmd := hyconExpandVariables(ctx, String(g.properties.Cmd))
